...
from wc_prediction.models import Teams, User, group_handler
...
def predict_group_stage():
    ...
    if request.method == "POST":
        ...
        if s_team_obj: # quick check if s_team_obj is query.first() result
            eks = group_handler.count(s_team_obj.group) # Logic for not advancing  more than 2 teams from group
            if eks < 2:
                s_team_obj.adding()
                group_handler.append(s_team_obj.group)
                flash(f"You selected {s_team_obj.qualified_teams} as team who will advance in group: {s_team_obj.group}!", category='success')
            else:
                flash(f"You already selected two teams to advance from group: {s_team_obj.group}!", category='danger')
        db.session.commit()
        ...
        if not_adv_team_obj:
            not_adv_team_obj.changing() # setting flag to 1 (Not advanced)
            group_handler.remove(not_adv_team_obj.group) # Logic for not advancin more than 2 teams from group
    ...
    if request.method == "GET":
        teams_to_pop = Teams.query.filter_by(advance_f = 2) # sending on the right side of the page all the objects of class Teams with advance_f (flag) == 2 (Advanced)
        advanced_teams = Teams.query.filter_by(advance_f = 1) # sending on the left side of the page all the objects of class Teams with advance_f (flag) == 1 (Not advanced)

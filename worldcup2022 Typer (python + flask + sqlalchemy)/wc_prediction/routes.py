...
def predict_group_stage():
    ...
    for primary_key, data_dict in enumerate(participating_teams):
        sql_request = "insert into Teams values ({}, '{}', '{}', {}, '{}')".format(primary_key, data_dict['group'], data_dict['qualified_teams'], current_user.id, "N")
        db.session.execute(sql_request)
        db.session.commit()
    
    # advanced_teams = Teams.query.all()
    ...
    if request.method == "POST":
        ...
        if s_team_obj:
            s_team_obj.adding(current_user)
            db.session.commit()
        ...
        if not_adv_team_obj:
            not_adv_team_obj.changing(current_user)
            db.session.commit()
    ...
    if request.method == "GET":
        teams_to_pop = Teams.query.filter(Teams.advance_f == "A")
        advanced_teams = Teams.query.filter(Teams.advance_f == "N")
